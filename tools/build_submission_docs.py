...
def build_function_doc(examples: dict) -> Path:
    ...
    add_table(doc, ["等级", "分数区间", "解释口径"], [
        ["L1 入门", "0–49", "能够识别部分常见概念，需要结构化指导和基础练习。"],
        ["L2 基础", "50–64", "能够完成常规单步任务，但复杂场景与风险控制不足。"],
        ["L3 熟练", "65–79", "能够拆解任务、组合方法并进行基本核验。"],
        ["L4 高级", "80–89", "能够设计流程、评测体系和风险控制机制。"],
        ["L5 专家", "90–100", "能够建立组织级标准并持续校准；须结合开放/实操与多次证据。"],
    ], [1.10, 1.25, 4.15])
    add_callout(doc, "解释边界", "L5 不以单道选择题直接判定。当前等级阈值属于产品初始口径，正式对外宣称测评效度前，应使用真实样本完成难度、区分度和等级一致性校准。", fill=LIGHT_RED)
    ...
    add_table(doc, ["指标", "当前值", "设计说明"], [
        ["总题量", "90题", "六维各15题，达到赛题每维10基础+5进阶的数量要求"],
        ["题型", "84选择 + 3开放 + 3实操", "每维至少包含一题非客观任务；后续扩展代码、图像题"],
        ["难度", "难度1：30；难度2：34；难度3：26", "难度由概念识别到复杂流程设计逐级提升"],
        ["答案位置", "A/B/C/D各21题", "正确选项位置完全均衡，降低位置偏差"],
        ["题目解析", "90/90", "提交后与报告复盘均可查看"],
...
def build_future_doc() -> Path:
    ...
    add_callout(doc, "规模不等于质量", "千题题库应以蓝图覆盖与统计质量为目标，不使用模板机械复制。当前90题可作为首个校准批次。")
