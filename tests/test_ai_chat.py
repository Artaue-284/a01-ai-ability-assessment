...
class AiAssistantUnitTests(unittest.TestCase):
    ...
    def test_clean_markup_reply_extracts_readable_agent_output(self):
        """回归：模型偶发输出工具调用/Agent 标记时，清洗为可读文本，不外显原始代码。"""
        assistant = AIAssistant()
        scripted = (
            "<tool_call>BFSM</tool_call>\n"
            "<arg_key>current_state</arg_key><arg_value>clarify_scope</arg_value>\n"
            "<arg_key>transitions</arg_key><arg_value>[{\"condition\": \"user_provides_scope\", \"target\": \"draft_checklist\"}]</arg_value>"
        )
        cleaned = assistant._clean_markup_reply(scripted)
        self.assertTrue(assistant._has_markup(scripted))
        self.assertNotIn("<tool_call>", cleaned)
        self.assertNotIn("</arg_value>", cleaned)
        self.assertNotIn("<arg_key>", cleaned)
        self.assertIn("clarify_scope", cleaned)
        # 纯标签、无任何文本内容时返回空串（交由调用方降级）
        self.assertEqual(assistant._clean_markup_reply("<tool_call></tool_call>"), "")
        # 只有 key 类标签、无 arg_value 时也剥成可读文本
        k = assistant._clean_markup_reply("<tool_call>BFSM</tool_call><arg_key>current_state</arg_key>")
        self.assertNotIn("<", k)
        self.assertIn("BFSM", k)

    def test_prompt_forbids_markup(self):
        """回归：系统/对话提示必须明确禁止输出结构化标记。"""
        assistant = AIAssistant()
        transcript = assistant._build_transcript({"question": "设计人机协同机制"}, [], "怎么做？")
        self.assertIn("禁止输出任何 XML/HTML 标签", transcript)
        self.assertIn("tool_call", transcript)
