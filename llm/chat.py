...
class AIAssistant:
    ...
    # 模型偶发输出的工具调用 / Agent 结构化标记（未启用 function calling 时被当作文本回复）
    _MARKUP_TAGS = re.compile(
        r"</?(tool_call|arg_key|arg_value|function_call|function|result|thought|reasoning|"
        r"observation|action|input|output|state|transition)>"
    )

    @staticmethod
    def _has_markup(reply: str) -> bool:
        return bool(re.search(r"<tool_call|<arg_key|<arg_value|<function_call|<result|<thought|</arg_", reply or ""))

    @classmethod
    def _clean_markup_reply(cls, reply: str) -> str:
        """剥离模型偶发输出的工具调用 / Agent 标记，保留可读文本。

        优先提取 <arg_value> 的内容拼接；否则剥离所有已知标记标签；
        若仍残留 <...> 标签一并去除。返回空串表示无有效文本（由调用方降级）。
        """
        text = (reply or "").strip()
        if not text:
            return text
        values = re.findall(r"<arg_value>([\s\S]*?)</arg_value>", text)
        readable = [v.strip() for v in values if v.strip()]
        if readable:
            return " ".join(readable)[:1000]
        cleaned = cls._MARKUP_TAGS.sub("", text)
        cleaned = re.sub(r"<[^>]+>", "", cleaned)          # 其他 XML/HTML 标签
        cleaned = re.sub(r"\n{3,}", "\n\n", cleaned).strip()
        return cleaned
    ...
    def _build_transcript(self, question: dict[str, Any], history: list[dict[str, str]], message: str) -> str:
        lines = [
            ...
            "直接输出中文自然语言，不要一次性输出多轮引导计划，不要使用\"第一轮/第二轮/等待学员回复\"等导演式注释，"
            "禁止输出任何 XML/HTML 标签、tool_call、arg_key、arg_value、代码块或结构化标记，"
    ...
    def _tbox_chat(self, question: dict[str, Any], history: list[dict[str, str]], message: str) -> dict[str, Any]:
        ...
        reply = self._single_turn_reply(reply)
        reply = self._clean_markup_reply(reply)
        if not reply:
            raise ValueError("百宝箱响应仅包含结构化标记，无可读文本")
    ...
    def _remote_chat(self, question: dict[str, Any], history: list[dict[str, str]], message: str) -> dict[str, Any]:
        ...
        reply = self._clean_markup_reply(reply)
        if not reply:
            raise ValueError("模型响应仅包含结构化标记，无可读文本")
    ...
    def _ant_line_chat(self, question: dict[str, Any], history: list[dict[str, str]], message: str) -> dict[str, Any]:
        ...
        system = (
            ...
            "然后等待学员的下一条消息。直接输出中文自然语言，绝对不要一次性输出多轮引导计划，"
        ...
        reply = self._clean_markup_reply(reply)
        if not reply:
            raise ValueError("ant-line 响应仅包含结构化标记，无可读文本")
