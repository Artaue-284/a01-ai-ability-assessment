...
class LLMScorer:
    ...
    def _parse_json_object(text: str) -> dict:
        ...
        # 模型偶发返回工具调用 / Agent 结构化标记而非 JSON——直接判为无效，交由外层降级
        if re.search(r"<tool_call|<arg_key|<arg_value|<function_call|<result|</arg_", cleaned):
            raise ValueError("模型响应为结构化标记而非评分 JSON")
        try:
