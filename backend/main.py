...
def load_admin_access_key() -> str:
    ...
    if configured:
        if len(configured) < 24:
            raise RuntimeError("A01_ADMIN_KEY must contain at least 24 characters")
        return configured
    if os.getenv("A01_REQUIRE_ENV_SECRETS", "0") == "1":
        raise RuntimeError("A01_ADMIN_KEY must be configured in public mode")
...
def load_enterprise_access_key() -> str:
    ...
    if configured:
        if len(configured) < 24:
            raise RuntimeError("A01_ENTERPRISE_KEY must contain at least 24 characters")
        return configured
    if os.getenv("A01_REQUIRE_ENV_SECRETS", "0") == "1":
        raise RuntimeError("A01_ENTERPRISE_KEY must be configured in public mode")
...
cors_origins = [item.strip() for item in os.getenv("A01_CORS_ORIGINS", "").split(",") if item.strip()]
if cors_origins:
    app.add_middleware(CORSMiddleware, allow_origins=cors_origins, allow_credentials=False, allow_methods=["GET", "POST", "PUT", "DELETE", "OPTIONS"], allow_headers=["Authorization", "Content-Type", "X-Admin-Key", "X-Enterprise-Key"])
allowed_hosts = [item.strip() for item in os.getenv("A01_ALLOWED_HOSTS", "").split(",") if item.strip()]
if allowed_hosts:
...
def startup() -> None:
    ...
    if os.getenv("A01_LOAD_DEMO_DATA", "0") == "1":
        ...
        seed_demo_positions_and_applications()
...
def upload_evidence(request: EvidenceUploadRequest):
    if load_test(request.test_id) is None:
        raise HTTPException(404, "测评不存在")
    question = question_index().get(request.question_id)
    if question is None:
        raise HTTPException(404, "题目不存在")
    ...
    safe_test_id = re.sub(r"[^A-Za-z0-9_-]", "_", request.test_id)
    folder = ROOT / "data" / "evidence" / safe_test_id
